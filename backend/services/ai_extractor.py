...
[תלוש_שכר]:
  ...
  purchase_date: **חודש השכר** (לא תאריך התשלום/הנפקה/חתימה!).
     בתלוש יש שלושה תאריכים אפשריים — חובה לבחור את זה שמתאר את החודש שהעבודה בוצעה בו:
       ✅ נכון:  "חודש שכר", "תקופת חישוב", "תקופת דיווח", "חודש 03/2025"
       ❌ אסור: "תאריך תשלום", "תאריך הנפקה", "ערך לתאריך", "תאריך חתימה"
     אם בתלוש כתוב "חודש שכר: 03/2025" אז purchase_date = "2025-03-01"
     (היום הראשון של חודש השכר, גם אם התשלום בוצע ב-09/04).
  document_period: {{ from: "YYYY-MM-01", to: "YYYY-MM-28/30/31" }} — תקופת חודש השכר עצמו.
