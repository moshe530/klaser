...
[תלוש_שכר]:
  ...
  ⚠️ **חילוץ התאריך — קריטי!** בתלוש שכר יש בדרך כלל 5-8 תאריכים שונים.
     **רק אחד מהם** הוא הנכון לשם המסמך: **חודש השכר** (החודש שעליו משולם).

     ✅ **התאריך שצריך לחלץ** מזוהה ע"י אחת התוויות/הסימנים הבאים:
        - תווית מפורשת: "חודש שכר", "חודש השכר", "שכר חודש", "שכר עבור"
        - "תקופת חישוב", "תקופת דיווח", "תקופת עבודה" (במשמעות חודש הדיווח)
        - "Salary for", "Pay period", "Period"
        - **פורמט מקוצר ליד הכותרת**: בתלושים ישראליים רבים אין תווית מפורשת,
          אלא רק תאריך מקוצר כמו `4/26`, `04/26`, `4-26`, `04-2026`, `4/2026`,
          `אפריל 26`, `April 2026` — מופיע **בקרבת הכותרת "תלוש משכורת"** או
          ב-header של הטופס. אם זה הפורמט היחיד הקיים, **השתמש בו** (אל תחזור
          ל-null כי "אין תווית"). פיענוח: `4/26` = "2026-04-01".
        - שדה דיווח לרשויות: "104/126 חודש 03/25"

     **הבחנה חשובה**: בתלוש ישראלי, התאריך הקצר (`4/26`) הוא חודש השכר,
     בעוד תאריך מלא בפורמט DD/MM/YYYY (כמו `01/08/2022`) הוא תמיד **לא**
     חודש השכר — אלא תחילת עבודה / ותק / הדפסה / וכד'.

     **🚨 איסור מוחלט על ערבוב תאריכים:**
     אם בתלוש יש `01/08/2022` (תחילת עבודה) ו-`4/26` (חודש שכר),
     **אסור** לשלב ולהחזיר `04/2022` או `2022-04-01` — זוהי "הזיה" (hallucination).
     החודש (`04`) והשנה (`26`→`2026`) חייבים להגיע **מאותו תאריך** בלבד.

     **כלל בריאות**: בדוק את שנת ההדפסה/תקופת ההפקה של המסמך —
     היא בדרך כלל מצביעה על שנת חודש השכר (או חודש אחריו).
     אם בתלוש "תאריך הדפסה: 17/05/2026" — שנת חודש השכר היא 2026, **לא** 2022.

     ❌ **תאריכים שאסור לחלץ — גם אם הם הבולטים ביותר במסמך:**
        - "תאריך לידה" / "ת. לידה" / "Date of Birth" — של העובד או מישהו אחר!
        - "תאריך תחילת עבודה" / "ותק" / "תאריך הצטרפות"
        - "תאריך הנפקת התלוש" / "תאריך הפקה" / "Issued on"
        - "תאריך תשלום" / "תאריך ערך" / "Payment date" / "Value date"
        - "תאריך אסיפה" / "תאריך בנק" / "Bank date"
        - "תאריך חתימה" / "תאריך עדכון"
        - "תוקף" / "תאריך תפוגה"
        - תאריכי תנועות בבנק או הפקדות לפנסיה שמופיעים בטבלאות.

     🔍 **אלגוריתם הזיהוי** (בצע בסדר):
        שלב 1: חפש את הכותרת של התלוש (לרוב בראש העמוד):
           דוגמאות מצויות: "תלוש משכורת לחודש 03/2025", "Payslip - March 2025"
        שלב 2: אם לא בכותרת — חפש שדה עם תווית "חודש שכר" או דומיה (ראה רשימה לעיל).
        שלב 3: אם יש רק "תקופה: 01/03/2025 - 31/03/2025" — בחר את תאריך הסיום.
        שלב 4: **אם אינך מוצא חודש שכר ברור — החזר null!**
           עדיף null מאשר תאריך לידה של העובד.

  purchase_date: היום הראשון של חודש השכר. דוגמה: "חודש שכר 03/2025" → "2025-03-01".
                 (גם אם התשלום בוצע ב-09/04 — purchase_date נשאר 2025-03-01).
  document_period: {{ from: "YYYY-MM-01", to: "YYYY-MM-28/30/31" }} — חודש השכר עצמו.
                   **גם purchase_date וגם document_period.from חייבים להצביע על אותו חודש!**
